...
```regexp
Какое последующее использование предполагается?
```
...
```regexp
Выражение "столь чудесного" очень субъективно. Cтоит удалить 
```
#### Критерии приёмки: 
...
| 2             | Количество карточек | Количество карточек ограничено 5. При превышении новые цитаты заменяют старые по правилам односторонней очереди |

```regexp
не указан максимальный и минимальный размер текста, пригодный для отображения и для внесения в базу данных
```
