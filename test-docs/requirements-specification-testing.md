...
```regexp
Непонятно сформулировано. Как именно происходит редактирование?
```
#### 3.8 User story:
...
```
#### 3.9 User story:
Как клиенту, мне нужна возможность получить пожелание счастливого пути перед 
закрытием приложеня
...
```
#### Критерии приёмки: 
Клиенты получают позитив при закрытии приложения
...
| 7 | Изменение цитат            | Изменение осуществляется двойным кликом на карточке и последующим редактированием во всплывающем окне                                                                                                                               |
...
3 Что значит "повторение цитаты"? Имеется в виду смысл цитаты или дословное совпадение
```
...
4 Всплывающее окно на какой странице? "Домой" или "О нас"?
Какое именно всплывающее окно из двух или оба?
...
Данные пересылаются по сети в формате JSON c обязательными полями id и content.
 

```regexp
В схеме API в запросе и ответе отсутствует поле id
